...
if (!acquired) {
  ...
  // Backend can also `process.exit(0)` in `full`; in `lite` just return early.
}

// React to a menu item click (menu items are declared in tynd.config.ts).
menu.onClick("file-open", async () => {
