...
function startResultsServer(): { wait: () => Promise<ResultsPayload>; stop: () => void } {
  ...
  const promise = new Promise<ResultsPayload>(r => { _resolve = r })
...
function printComparison(
  ...
  console.log(`  ${bold("Performance Comparison")}   ${dim("(values: P50 for latency, perCall for concurrent, ops/s for sustained)")}`)
  console.log()
  ...
  for (const liteRow of liteRes.rows) {
    ...
    if (liteRow.kind === "na") {
      ...
      const name = liteRow.name.length > COL[0]
    ...
    if (liteRow.kind === "cold" && liteRow.coldMs !== undefined && fullRow.coldMs !== undefined) {
      liteVal = liteRow.coldMs; fullVal = fullRow.coldMs
      liteStr = fmt(liteVal);   fullStr = fmt(fullVal)
    } else if (liteRow.kind === "latency" && liteRow.p50 !== undefined && fullRow.p50 !== undefined) {
      liteVal = liteRow.p50; fullVal = fullRow.p50
      liteStr = fmt(liteVal); fullStr = fmt(fullVal)
    } else if (liteRow.kind === "concurrent" && liteRow.perCall !== undefined && fullRow.perCall !== undefined) {
      liteVal = liteRow.perCall; fullVal = fullRow.perCall
      liteStr = fmt(liteVal);    fullStr = fmt(fullVal)
    } else if (liteRow.kind === "sustained" && liteRow.ops !== undefined && fullRow.ops !== undefined) {
      liteVal = liteRow.ops; fullVal = fullRow.ops
      liteStr = fmtOps(liteVal); fullStr = fmtOps(fullVal)
      lowerIsBetter = false
    }
    ...
    const ratioStr = ratio < TIE_THRESHOLD ? ""
      : ratio < 10  ? `+${Math.round((ratio - 1) * 100)}%`
      :               `${ratio.toFixed(1)}×`
    ...
    const liteDisplay = liteWins_this
      ...
      : isTie ? dim(liteStr.padStart(COL[1])) : red(liteStr.padStart(COL[1]))
    const fullDisplay = fullWins_this
      ? green(fullStr.padStart(COL[2]))
      : isTie ? dim(fullStr.padStart(COL[2])) : red(fullStr.padStart(COL[2]))
  ...
  console.log(`  ${dim("∞  = full-exclusive (file I/O, SQLite)   ~  = noisy measurement (CV > 20%)")}`)
  console.log()
  ...
  const d200row = liteRes.rows.find(r => r.name === "filter+sort list — 200 items")
  ...
  console.log()
  console.log(`  ${bold("Button clicks")}        Both modes imperceptible — ${fmt(liteIPC)} (lite) vs ${fmt(fullIPC)} (full). Users won't notice.`)
  if (coldFull > 0 && coldLite > 0) {
    const r = coldFull / coldLite
    const coldRatioStr = r >= 10 ? `${r.toFixed(1)}×` : `+${Math.round((r - 1) * 100)}%`
    console.log(`  ${bold("App cold start")}       Lite ${coldRatioStr} faster first call (${fmt(coldLite)} vs ${fmt(coldFull)}). Noticeable on slower hardware.`)
  }
  if (d2kLiteV > 0 && d2kFullV > 0) {
    const pct = Math.round(((d2kLiteV / d2kFullV) - 1) * 100)
    console.log(`  ${bold("Large data (2k items)")} Full ${pct}% faster (${fmt(d2kFullV)} vs ${fmt(d2kLiteV)}). Both still < 10ms — invisible to users.`)
  }
  if (p100LiteV > 0 && p100FullV > 0) {
    const pct = Math.round(((p100LiteV / p100FullV) - 1) * 100)
    console.log(`  ${bold("100KB payloads")}       Full ${pct}% faster (${fmt(p100FullV)} vs ${fmt(p100LiteV)}). Noticeable only for frequent large transfers.`)
  }
  if (susLiteN > 0 && susFull_N > 0) {
    const pct = Math.round(((susLiteN / susFull_N) - 1) * 100)
    console.log(`  ${bold("Sustained throughput")} Lite ${pct}% more calls/s (${fmtOps(susLiteN)} vs ${fmtOps(susFull_N)}). Relevant for tight polling loops only.`)
  }
  console.log(`  ${bold("File I/O, SQLite")}     Full-exclusive — lite has no filesystem access. Use full if your app reads/writes files.`)
  console.log(`  ${bold("CPU-heavy work")}        Full wins significantly (40× for fib, 11× for regex). Lite adequate for data transforms < 500 items.`)
  console.log()
}
