...
function logConcurrent(
  rows: BenchRow[],
  name: string,
  r: { total: number; perCall: number; ops: number },
) {
...
async function runBenchmark(): Promise<void> {
  ...
  try {
    ...
    logLatency(
      rows,
      "empty call — with result check",
      await bench(() => api.ping().then((r) => r === "pong"), 10, 200),
    )
    ...
    logLatency(
      rows,
      "filter+sort list — 200 items",
      await bench(() => api.processDataset(200), 5, 50),
    )
    logLatency(
      rows,
      "filter+sort list — 2 000 items",
      await bench(() => api.processDataset(2_000), 5, 30),
    )
    logLatency(
      rows,
      "validate API batch — 50 items",
      await bench(() => api.processApiResponse(50), 5, 100),
    )
    logLatency(
      rows,
      "validate API batch — 500 items",
      await bench(() => api.processApiResponse(500), 5, 50),
    )
    ...
    if (!caps.fileIO) {
      ...
      logLatency(
        rows,
        "settings roundtrip — write+read 1KB",
        await bench(() => api.fileRoundtrip(1), 3, 30),
      )
    }
    ...
    if (!caps.sqlite) {
      ...
      logLatency(
        rows,
        "db list top 100 results",
        await bench(() => api.sqliteSelectRange(100), 5, 100),
      )
      logLatency(
        rows,
        "db save 100 records (batch)",
        await bench(() => api.sqliteInsertBatch(100), 3, 30),
      )
      logLatency(rows, "db dashboard aggregate", await bench(() => api.sqliteAggregate(), 5, 100))
      logLatency(
        rows,
        "db full-text search (LIKE)",
        await bench(() => api.sqliteSearch("item-1"), 5, 100),
      )
    }
    ...
    logLatency(
      rows,
      "send+receive 1KB",
      await bench(() => api.echoPayload({ data: "x".repeat(900) }), 5, 100),
    )
    logLatency(
      rows,
      "send+receive ~10KB",
      await bench(
        () => api.echoPayload(Array.from({ length: 200 }, (_, i) => ({ id: i, v: Math.PI }))),
        5,
        50,
      ),
    )
    logLatency(
      ...
      "send+receive ~100KB",
      await bench(
        () => api.echoPayload(Array.from({ length: 2000 }, (_, i) => ({ id: i, v: Math.PI }))),
        3,
        20,
      ),
    )
    ...
    logLatency(
      rows,
      "nested config object (depth 10)",
      await bench(() => api.deepObject(10), 5, 100),
    )
    ...
    for (const n of [10, 50] as const) {
      ...
      logConcurrent(
        rows,
        `${n} simultaneous calls with 1KB payload`,
        await benchConcurrent(() => api.echoPayload(payload), n),
      )
    }
    ...
    logSustained(
      rows,
      "max call rate with small payload",
      await benchSustained(() => api.echoPayload({ x: 42 }), 2000),
    )
    ...
    logLatency(
      rows,
      "[engine] fibonacci(25) — recursive",
      await bench(() => api.fibonacci(25), 3, 20),
    )
    logLatency(
      rows,
      "[engine] fibonacci(30) — recursive",
      await bench(() => api.fibonacci(30), 2, 5),
    )
    logLatency(
      rows,
      "[engine] sort 1 000 numbers",
      await bench(() => api.sortInPlace(1_000), 5, 50),
    )
    logLatency(
      rows,
      "[engine] sort 10 000 numbers",
      await bench(() => api.sortInPlace(10_000), 5, 20),
    )
    logLatency(
      rows,
      "[engine] map→filter→reduce 1 000 items",
      await bench(() => api.arrayTransform(1_000), 5, 50),
    )
    logLatency(
      rows,
      "[engine] map→filter→reduce 10 000 items",
      await bench(() => api.arrayTransform(10_000), 5, 20),
    )
    logLatency(
      rows,
      "[engine] JSON stringify+parse ×100",
      await bench(() => api.jsonWork(100), 5, 50),
    )
    logLatency(rows, "[engine] regex match ×100", await bench(() => api.regexWork(100), 5, 50))
    logLatency(
      rows,
      "[engine] string concat ×10 000",
      await bench(() => api.stringWork(10_000), 5, 30),
    )
    logLatency(
      rows,
      "[engine] object alloc ×1 000",
      await bench(() => api.objectAlloc(1_000), 5, 50),
    )
    logLatency(
      rows,
      "[engine] object alloc ×10 000",
      await bench(() => api.objectAlloc(10_000), 5, 20),
    )
    ...
    try {
      ...
      if (mem.rssMB > 0) {
        backendHeapMB = mem.heapUsedMB
        backendRssMB = mem.rssMB
      }
    } catch {
      /* lite — not available */
    }
