...
- Sign the binary — see the [Code Signing guide](/docs/v0.2/guides/code-signing).
...
- Sign + notarize — see [Code Signing](/docs/v0.2/guides/code-signing).
...
`tynd.config.ts` is validated with [valibot](https://valibot.dev). The error message lists the offending field. Required: `runtime`. See the [config reference](/docs/v0.2/cli/config).
...
React Compiler (React 19+) conflicts with Vite's Fast Refresh. Either disable the Compiler in dev mode, or accept full reloads. See [Frontend Frameworks](/docs/v0.2/guides/frontend-frameworks).
