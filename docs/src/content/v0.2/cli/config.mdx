...
<Callout type="info">
  **Window / menu / tray config lives in the backend**, not in `tynd.config.ts`. Each spawned window can configure itself programmatically, so `app.start({ window: {...}, menu: [...], tray: {...} })` is the authoritative place. See [Backend API](/docs/v0.2/api/backend).
</Callout>
...
See the [Deep Linking guide](/docs/v0.2/guides/deep-linking).
...
Rationale: each spawned window can define its own settings programmatically; a static config file can't express that flexibility. See [Backend API](/docs/v0.2/api/backend).
...
- [Backend API](/docs/v0.2/api/backend) — `app.start`, window/menu/tray config.
