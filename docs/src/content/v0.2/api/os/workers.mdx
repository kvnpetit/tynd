...
For **Rust-backed compute** (hashing, CSPRNG), prefer [`compute`](/docs/v0.2/api/os/compute) — it's faster and doesn't round-trip through JSON.
...
- [compute](/docs/v0.2/api/os/compute) — Rust-native hashing and RNG (faster for those tasks).
- [Streaming RPC](/docs/v0.2/api/streaming-rpc) — if you need to stream intermediate results.
