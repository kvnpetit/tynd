...
If the postinstall was skipped or failed (old `@tynd/host` version, network glitch), `tynd dev` / `start` / `build` / `validate` fall back to fetching the matching release asset on demand and cache it under `node_modules/@tynd/host/bin/<platform>-<arch>/`. If that also fails, see [Troubleshooting](/docs/v0.1/troubleshooting).
...
- [Your First App](/docs/v0.1/getting-started/first-app)
