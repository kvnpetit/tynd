...
- **Buffered output.** stdout/stderr are fully captured and returned at the end — not suitable for long-running processes that print steadily (memory grows). Use [`terminal.spawn`](/docs/v0.1/api/os/terminal) for streaming.
...
- [terminal](/docs/v0.1/api/os/terminal) — real PTY, streaming stdout.
- [sidecar](/docs/v0.1/api/os/sidecar) — resolve a bundled binary's runtime path.
- [shell](/docs/v0.1/api/os/shell) — OS default-handler launches.
