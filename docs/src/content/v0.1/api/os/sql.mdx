...
`BLOB` columns come back as **base64 strings** over the JSON IPC channel. For large blobs, store a path in the table and use [`fs.readBinary`](/docs/v0.1/api/os/fs) on the payload.
...
- [store](/docs/v0.1/api/os/store) — k/v, simpler for flat preferences.
- [fs](/docs/v0.1/api/os/fs) — for large BLOBs stored out-of-row.
