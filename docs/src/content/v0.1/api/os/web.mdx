...
- [`http`](/docs/v0.1/api/os/http) — request retries, download-with-progress
  events, no CORS.
- [`websocket`](/docs/v0.1/api/os/websocket) — dedicated thread per session,
  auto-reconnect helpers.
- [`compute`](/docs/v0.1/api/os/compute) — `hash` (blake3/sha2) and
  `randomBytes` via Rust, ~3–10× faster than `crypto.subtle` on lite.

## Related

- [Runtimes](/docs/v0.1/runtimes) — full vs lite parity matrix.
- [Architecture](/docs/v0.1/concepts/architecture) — where polyfills fit.
