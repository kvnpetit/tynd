  ...
  test:
    ...
    steps:
      ...
      - name: Linux WebView deps
        if: runner.os == 'Linux'
        run: sudo apt install -y libgtk-3-dev libwebkit2gtk-4.1-dev libjavascriptcoregtk-4.1-dev libsoup-3.0-dev libxdo-dev libxss-dev
      - run: bun install
      - run: bun test
      - run: bunx tynd build
...
- [Performance](/docs/v0.3/guides/performance) — profiling and optimization.
- [Debugging](/docs/v0.3/guides/debugging) — devtools, logs, verbose mode.
