...
No. Desktop only. If you need mobile, look at [Tauri v2](/docs/v0.3/compare/vs-tauri) (its big differentiator).
...
Phase-1 features (OS APIs, IPC, streaming RPC, TYNDPKG, installers, signing, updater) are solid and tested — see [Production Checklist](/docs/v0.3/production-checklist). Some advanced surfaces (mobile, cross-compile, macOS `.app` auto-updater install, capability ACLs) aren't implemented. See the [vs Tauri/Electron matrix](/docs/v0.3/compare) for specifics.
...
Same IPC stack (`wry` + `tao`). Different backend language — TypeScript vs Rust. Smaller binary option (`lite`). No mobile, no capability ACLs, no 30-plugin ecosystem. Full detail: [vs Tauri](/docs/v0.3/compare/vs-tauri).

## How does it compare to Electron?

~25× smaller binary. Native WebView, not bundled Chromium. Zero-codegen typed RPC out of the box. No `desktopCapturer`, no `printToPDF`, no Chrome extensions — those are Chromium-bundled features. Full detail: [vs Electron](/docs/v0.3/compare/vs-electron).

## How do I sign my app?

Declare `bundle.sign` in `tynd.config.ts`. Windows uses `signtool`, macOS uses `codesign` + optional `notarytool`. See the [Code Signing guide](/docs/v0.3/guides/code-signing).

## How does auto-update work?

Ed25519-signed manifests (Tauri-compatible format). Tynd ships `tynd keygen` / `tynd sign` — no third-party signing tool needed. The public key is baked into your app; a compromised manifest server can't serve an unverifiable binary. See the [Auto-Updates guide](/docs/v0.3/guides/auto-updates).

## Can I bundle a native binary (ffmpeg, etc.)?

Yes — `sidecars` in `tynd.config.ts`. See [Sidecars guide](/docs/v0.3/guides/sidecars) and the [Embed a Sidecar recipe](/docs/v0.3/recipes/embed-sidecar).
...
Use [`keyring`](/docs/v0.3/api/os/keyring) — OS Keychain / Credential Manager / Secret Service. Never put tokens / passwords / session cookies in [`store`](/docs/v0.3/api/os/store) (plain JSON on disk).
...
Ship a "Export diagnostics" feature: output of `os.info()`, `app.getVersion()`, recent logs from a file sink. See the [Debugging guide](/docs/v0.3/guides/debugging).
...
No. `tynd://` + `tynd-bin://` are native custom schemes — no TCP, no loopback port, no firewall prompt. See [IPC model](/docs/v0.3/concepts/ipc).

## How do I test my app?

`bun test` for unit tests. Mock OS APIs at the module boundary. Smoke-test the built binary in CI (spawn, wait 2 s, expect no crash). No Playwright / WebdriverIO support — `tynd://` isn't a browser-reachable URL. See [Testing guide](/docs/v0.3/guides/testing).
