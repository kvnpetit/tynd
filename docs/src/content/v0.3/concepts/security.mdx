...
For fine-grained permission ACLs (capability system), Tynd now ships an opt-in [`security` API](/docs/v0.3/api/os/security) with glob allow / deny rules and a default-deny mode — guarding every `fs.*` call, `http.request`/`download`, and navigation in the WebView.
...
## Capability ACL (`security`) — opt-in since v0.3

```ts
import { security } from "@tynd/core/client";

await security.configure({
  fs:   { allow: ["${dataDir}/myapp/**"], deny: ["**/.ssh/**"] },
  http: { allow: ["https://api.myapp.com/**"] },
  defaultDeny: true,
});
```

Deny beats allow. Globs: `*` (intra-segment) + `**` (cross-segment). Paths normalized; case-insensitive on Windows. Full reference: [security API](/docs/v0.3/api/os/security).
