...
Both runtimes expose the **same 30 Tynd OS APIs** (`fs`, `http`, `sql`, `process`, `store`, `compute`, `workers`, `terminal`, `sidecar`, `singleInstance`, `dialog`, `clipboard`, `shell`, `notification`, `tray`, `tyndWindow`, `menu`, `shortcuts`, `keyring`, `autolaunch`, `monitor`, `updater`, `websocket`, `app`, `os`, `path`, `log`, `power`, `security`) via `@tynd/core/client`. They're Rust-backed and behave identically.
...
**All identical on both runtimes** — implemented in Rust, called the same way from the same `@tynd/core/client` import. See the [API Reference](/docs/v0.3/api) for the complete list.
...
- [Full runtime reference page](/docs/v0.3/runtimes)
- [Compare Tynd vs other frameworks](/docs/v0.3/compare)
