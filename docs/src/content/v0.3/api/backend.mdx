...
| `quitOnLastWindowClosed` | `boolean` (default `false`) | Exit when the last window closes. Leave off for tray-only apps that should stay alive in the background. |
...
| `userAgent` | — | Custom User-Agent string set on every webview request. |
...
## Frontend -> backend events (`onFrontendEmit` + `createFrontendEmitter`)

The mirror of `createEmitter`: frontend emits, backend listens. Reuses the same RPC pipe — no new transport.

```ts filename="backend/main.ts"
import { onFrontendEmit } from "@tynd/core";

onFrontendEmit<string>("toast", (text) => log.info(text));
```

```ts filename="src/toast.ts"
...
Run raw JavaScript in a webview from the backend. When `label` is omitted the script runs on every open window.
...
React to clicks from the frontend with the [`menu`](/docs/v0.3/api/os/menu) API:
...
Handle clicks with the [`tray`](/docs/v0.3/api/os/tray) API:
...
- `async function*` backend handlers return [StreamCall](/docs/v0.3/api/streaming-rpc) handles — awaitable + async-iterable.

## Next

- [Streaming RPC](/docs/v0.3/api/streaming-rpc)
- [Backend RPC guide](/docs/v0.3/guides/backend-rpc)
- [OS API reference](/docs/v0.3/api)
