...
Use the standard accelerator syntax's format — same as [`shortcuts`](/docs/v0.3/api/os/shortcuts):
...
Show a native popup anchored on the current window. `x` / `y` are window-relative logical pixels; omit both to anchor on the cursor. Clicks feed the same `menu:action` event as menu bar + tray.
...
- [tray](/docs/v0.3/api/os/tray) — tray menu handlers share the same `menu.onClick` dispatcher.
- [shortcuts](/docs/v0.3/api/os/shortcuts) — global hotkeys (fire unfocused).
