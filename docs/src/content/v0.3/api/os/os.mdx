...
## `os.resourceDir(): Promise<string | null>`

Directory of the running binary — use it to locate resources packed next to the executable.

```ts
const res = await os.resourceDir();
const configPath = await path.join(res!, "defaults.json");
```
...
- [path](/docs/v0.3/api/os/path) — `path.join` to compose per-app paths.
- [store](/docs/v0.3/api/os/store) — uses `configDir` automatically.
