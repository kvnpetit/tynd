...
## `revealInFolder(path): Promise<void>`

Open the OS file manager with `path` selected — Windows `explorer /select`, macOS `open -R`. On Linux opens the parent directory (no standard "select" gesture across file managers).

```ts
await shell.revealInFolder("/Users/me/Downloads/report.pdf");
```
...
- [process](/docs/v0.3/api/os/process) — `process.exec` for more control over what's launched.
