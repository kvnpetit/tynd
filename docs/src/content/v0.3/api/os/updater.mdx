...
interface UpdaterCheckOptions {
  ...
  proxy?:           string;
  /** Accept manifests that advertise an older version. */
...
Run `check` on a timer and react to `updater:check` events. Single task at a time — calling `startPeriodicCheck` twice returns the existing id.

```ts
const id = await updater.startPeriodicCheck({
  endpoint:       "https://releases.example.com/update.json",
  currentVersion: pkg.version,
  intervalMs:     60 * 60 * 1000,   // 1 hour (min 1 s)
});

updater.onCheck((event) => {
  if (event.ok && "available" in event.info === false) return;
  notifyUserAboutUpdate(event);
});

// later
await updater.stopPeriodicCheck();
```
...
- [`tynd sign`](/docs/v0.3/cli/sign) — sign a file.
