...
pub fn dispatch(win: &Window, method: &str, args: &Value) -> Result<Value, String> {
    match method {
        ...
        "minimize"   => { win.set_minimized(true);  Ok(Value::Null) }
        "unminimize" => { win.set_minimized(false); Ok(Value::Null) }
        "maximize"   => { win.set_maximized(true);  Ok(Value::Null) }
        "unmaximize" => { win.set_maximized(false); Ok(Value::Null) }
        "center"     => { window::center_window(win); Ok(Value::Null) }

        "setFullscreen" => {
            let full = args.get("fullscreen").and_then(|f| f.as_bool()).unwrap_or(false);
            win.set_fullscreen(
                if full { Some(tao::window::Fullscreen::Borderless(None)) } else { None }
            );
            Ok(Value::Null)
        }

        "setAlwaysOnTop" => {
            let always = args.get("always").and_then(|a| a.as_bool()).unwrap_or(false);
            win.set_always_on_top(always);
            Ok(Value::Null)
        }

        "setDecorations" => {
            let dec = args.get("decorations").and_then(|d| d.as_bool()).unwrap_or(true);
            win.set_decorations(dec);
            Ok(Value::Null)
        }

        "show" => { win.set_visible(true);  Ok(Value::Null) }
        "hide" => { win.set_visible(false); Ok(Value::Null) }
