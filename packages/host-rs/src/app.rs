...
enum UserEvent {
    ...
    PageReady,
    OsResult    { id: String, ok: bool, value: Value },
    OsEvent     { name: String, data: Value },
    WindowCmd   { id: String, method: String, args: Value },
...
pub fn run_app(bridge: BackendBridge, debug: bool) -> ! {
    let BackendBridge { config, call_tx, event_rx } = bridge;
    ...
    // Build native window
    let native_window = window::build_window(&config.window, config.icon_path.as_deref(), &event_loop);
    ...
    let _app_menu: Option<muda::Menu> = if !config.menu.is_empty() {
        match build_muda_bar(&config.menu) {
            ...
            Err(e) => { crate::vorn_log!("Menu build failed: {e}"); None }
        }
    } else {
        None
    };
    ...
    let _system_tray: Option<tray_icon::TrayIcon> =
        config.tray.as_ref().and_then(|tc| {
            match build_tray(tc) {
                Ok(tray) => {
                    let proxy = proxy.clone();
                    tray_icon::TrayIconEvent::set_event_handler(
                        Some(move |evt: tray_icon::TrayIconEvent| {
                            let name = match &evt {
                ...
                Err(e) => { crate::vorn_log!("System tray failed: {e}"); None }
            }
        });
    ...
    let mut wb = WebViewBuilder::new_with_web_context(&mut web_context)
        ...
        .with_ipc_handler(move |req: Request<String>| {
            ...
            // ── Fast path: backend call ───────────────────────────────────────
            // The IPC shim always produces {"type":"call",...} via JSON.stringify
            // (compact, no spaces, keys in insertion order). Detect this without
            // a full JSON parse and forward the raw string directly to Bun,
            // eliminating one parse+serialize cycle per call.
            if body.starts_with(r#"{"type":"call""#) {
                let _ = call_tx_ipc.send(BackendCall::Raw(body.to_owned()));
                return;
            }

            // ── Single parse for all other message types ──────────────────────
            let Ok(v) = serde_json::from_str::<Value>(body) else { return };

            // Page-ready signal (fired once on DOMContentLoaded)
            if v.get("__vorn_page_ready").and_then(|f| f.as_bool()).unwrap_or(false) {
    ...
    let webview = wb.build(&native_window).expect("WebViewBuilder::build failed");
        ...
        match event {
            ...
            Event::UserEvent(UserEvent::Backend(evt)) => match evt {
                BackendEvent::Return { id, ok, value } => {
                    ...
                    if id.starts_with("__vorn_") { return; }
...
fn webview_data_dir(app_name: &str) -> Option<std::path::PathBuf> {
    // Build a safe directory name: keep alphanumerics and underscores,
    // collapse other chars to single dashes, trim leading/trailing dashes.
    ...
    while safe.ends_with('-') { safe.pop(); }
    if safe.is_empty() { safe = "vorn".to_string(); }
...
fn init_menu_bar(menu: &muda::Menu, window: &tao::window::Window) {
    #[cfg(target_os = "windows")]
    {
        ...
        unsafe {
            if let Err(e) = menu.init_for_hwnd(window.hwnd() as isize) {
