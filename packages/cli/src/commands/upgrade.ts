...
export async function upgrade(opts: UpgradeOptions): Promise<void> {
  ...
  log.step(`@vorn/cli   ${log.gray(cliCurrent)}      → ${compareSemver(cliLatest,  cliCurrent)      > 0 ? log.cyan(cliLatest)  : log.gray(cliLatest)}`)
  log.step(`@vorn/core  ${log.gray(coreCurrentStr)} → ${compareSemver(coreLatest, coreCurrentStr) > 0 ? log.cyan(coreLatest) : log.gray(coreLatest)}`)
  ...
  for (const pkg of runtimePkgs) {
    ...
    if (latest) {
      log.step(`${pkg}  ${log.gray(current)} → ${compareSemver(latest, current) > 0 ? log.cyan(latest) : log.gray(latest)}`)
  ...
  const corePkgsNeedUpgrade =
    compareSemver(cliLatest,  cliCurrent)      > 0 ||
    compareSemver(coreLatest, coreCurrentStr) > 0
