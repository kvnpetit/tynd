...
import { vornConfig, backendMain } from "../lib/template.ts"
...
export async function init(opts: InitOptions): Promise<void> {
  ...
  if (!hasFrontend) {
    await write(opts.cwd, "frontend/index.html",
`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
`, opts.force)
    ...
    await write(opts.cwd, "frontend/main.ts",
...
async function patchPackageJson(pkgPath: string, pkg: Record<string, unknown>): Promise<void> {
  ...
  log.step(`${log.cyan("patch")}   package.json (scripts: dev, build — deps: @vorn/core, @vorn/host)`)
}
