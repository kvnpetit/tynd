...
import { buildLiteBundle, buildFullBundle, buildFrontendEntry } from "../lib/bundle.ts"
...
export async function build(opts: BuildOptions): Promise<void> {
  ...
  const backendHash = hashSources(
    [backendSrcDir],
    [
      path.join(opts.cwd, "vorn.config.ts"),
      path.join(opts.cwd, "package.json"),
    ],
  )
  ...
  if (cfg.runtime === "lite") {
    await packageLite({ hostBin, backendBundle: cachedBundle, frontendFiles, outFile, platform, iconPath })
  } else {
    await packageFull({ hostBin, backendBundle: cachedBundle, frontendFiles, outFile, platform, iconPath })
  }
...
async function packageLite(o: LitePackOpts): Promise<void> {
  ...
  // Clean up temp icon
  if (tmpIconPath) try { rmSync(tmpIconPath) } catch {}
...
async function packageFull(o: FullPackOpts): Promise<void> {
  ...
  if (tmpBunDir) try { rmSync(tmpBunDir, { recursive: true }) } catch {}
