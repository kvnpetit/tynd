...
cli
  ...
  .action(async (name: string | undefined, opts: { framework?: string; runtime?: string }) => {
    ...
    const resolvedName = name
      ?? await (async () => { log.blank(); log.info("Create a new Vorn project"); log.blank(); return text("Project name", "my-app") })()

    const resolvedFw: Framework = (FRAMEWORKS.find(f => f.value === opts.framework)?.value as Framework | undefined)
