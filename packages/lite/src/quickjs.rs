...
use vorn_host::{runtime::{BackendBridge, BackendCall, BackendConfig, BackendEvent, MenuItemDef, TrayConfig}, vorn_log};
...
pub fn start(
    ...
    if frontend_dir.is_some() { config.frontend_dir = frontend_dir; }
    if dev_url.is_some()      { config.dev_url      = dev_url; }
    if icon_path.is_some()    { config.icon_path    = icon_path; }
...
fn js_thread_main(
    ...
    // ── Timer subsystem: single thread, priority-queue based ──────────────────
    // Eliminates spawning one OS thread per setTimeout/setInterval call.
    let (timer_tx, timer_rx) = mpsc::sync_channel::<TimerCmd>(64);
    start_timer_thread(timer_rx, js_tx.clone());

    // ── Inject globals + eval bundle ──────────────────────────────────────────
    {
        ...
        let result = ctx.with(|ctx| -> rquickjs::Result<()> {
            ...
            // console → eprintln
            g.set("__vorn_log__", Function::new(ctx.clone(), |level: String, msg: String| {
                eprintln!("[{level}] {msg}");
            })?)?;
            ...
            // __vorn_emit__(name, payloadJson) — called by app.start() emit
            {
                ...
                g.set("__vorn_emit__", Function::new(ctx.clone(), move |name: String, payload_json: String| {
            ...
            // __vorn_set_interval__(id, ms, once) — called by JS setTimeout/setInterval
            {
                ...
                g.set("__vorn_set_interval__", Function::new(ctx.clone(), move |id: u32, ms: u32, once: bool| {
            ...
            // __vorn_clear_interval__(id) — called by JS clearTimeout/clearInterval
            {
                ...
                g.set("__vorn_clear_interval__", Function::new(ctx.clone(), move |id: u32| {
    ...
    while let Ok(msg) = js_rx.recv() {
        match msg {
            JsMsg::Call(call) => {
                ...
                let evt = match result {
                    Ok(value)  => BackendEvent::Return { id, ok: true,  value },
                    Err(error) => BackendEvent::Return { id, ok: false, value: Value::String(error) },
                };
        ...
        while rt.is_job_pending() {
            match rt.execute_pending_job() {
                Ok(true)  => {}
                Ok(false) => break,
                Err(e) => { vorn_log!("Job error: {e}"); }
            }
        }
    }
}

// ── Timer thread — single thread, priority queue ──────────────────────────────
//
// Replaces the previous approach of spawning one OS thread per timer.
// Uses a min-heap ordered by fire_at and recv_timeout to sleep until the
// next deadline without busy-waiting.

fn start_timer_thread(cmd_rx: mpsc::Receiver<TimerCmd>, js_tx: mpsc::Sender<JsMsg>) {
    std::thread::spawn(move || {
        #[derive(Eq, PartialEq)]
        struct Entry { fire_at: Instant, id: u32, ms: u32, once: bool }

        // Ord reversed so BinaryHeap (max-heap) behaves as a min-heap on fire_at
        impl Ord for Entry {
            fn cmp(&self, o: &Self) -> std::cmp::Ordering { o.fire_at.cmp(&self.fire_at) }
        }
        ...
        loop {
            ...
            while heap.peek().map_or(false, |e| e.fire_at <= now) {
                ...
                if js_tx.send(JsMsg::TimerFire(e.id)).is_err() { return; }
                if !e.once {
                    // Re-schedule interval using fire_at + dur to prevent drift
                    let dur = Duration::from_millis(e.ms as u64);
                    heap.push(Entry { fire_at: e.fire_at + dur, id: e.id, ms: e.ms, once: false });
            ...
            match cmd_rx.recv_timeout(timeout) {
                Ok(TimerCmd::Set { id, ms, once }) => {
                    ...
                    heap.push(Entry { fire_at: Instant::now() + dur, id, ms, once });
                    active.insert(id);
                }
...
fn call_module_fn(ctx: &rquickjs::Ctx<'_>, fn_name: &str, args: &[Value]) -> Result<Value, String> {
    let do_call: Function = ctx.globals()
        .get("__vorn_do_call__")
        .map_err(|e| format!("__vorn_do_call__ missing: {e}"))?;

    let fn_js   = rquickjs::String::from_str(ctx.clone(), fn_name)
    ...
    let args_js   = rquickjs::String::from_str(ctx.clone(), &args_json)
        .map_err(|e| e.to_string())?;
...
fn call_global(ctx: &rquickjs::Ctx<'_>, fn_name: &str) -> Result<Value, String> {
    let fn_val: rquickjs::Value = ctx.globals()
        .get(fn_name)
        .map_err(|e| format!("global '{fn_name}' not found: {e}"))?;

    if fn_val.is_undefined() { return Ok(Value::Null); }
    if !fn_val.is_function() { return Ok(Value::Null); }

    let func: Function = fn_val.into_function().unwrap();
    let raw = func.call::<_, rquickjs::Value>(()).map_err(|e| e.to_string())?;

    if raw.is_undefined() || raw.is_null() { return Ok(Value::Null); }
    ...
    let s = result.and_then(|s| s.to_string().ok()).unwrap_or_else(|| "null".into());
