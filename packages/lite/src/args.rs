    ...
    pub fn parse() -> Self {
        ...
        while i < raw.len() {
            match raw[i].as_str() {
                "--bundle" | "-b" => {
                    i += 1;
                    if i < raw.len() { bundle_path = raw[i].clone(); }
                }
                "--frontend-dir" | "-f" => {
                    i += 1;
                    if i < raw.len() { frontend_dir = Some(raw[i].clone()); }
                }
                "--dev-url" => {
                    i += 1;
                    if i < raw.len() { dev_url = Some(raw[i].clone()); }
                }
                "--debug" | "-d" => debug = true,
                _ => {}
            }
        ...
        Self { bundle_path, frontend_dir, dev_url, debug }
    }
}
