...
pub fn try_load_embedded() -> Option<EmbeddedAssets> {
    ...
    if size < TRAILER_LEN { return None; }
    ...
    if size < TRAILER_LEN + section_size { return None; }

    // ── 3. Seek to start of packed section ────────────────────────────────────
    ...
    // ── 5. Create temp directory for non-cached assets ────────────────────────
    ...
    // ── 6. Derive app name for cache path (exe file stem) ─────────────────────
    let app_name = exe
        .file_stem()
    ...
    for _ in 0..file_count {
        ...
        match rel.as_str() {
            // ── bun.version: compute cache path, determine cache hit ──────────
            "bun.version" => {
            ...
            // ── bun.gz: decompress to cache on miss, skip on hit ──────────────
            "bun.gz" => {
            ...
            // ── bundle.js: write as-is to temp dir ───────────────────────────
            "bundle.js" => {
            ...
            // ── frontend/<path>: write preserving subdirs ─────────────────────
